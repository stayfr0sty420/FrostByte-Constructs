...
2. Create `.env` (don’t commit it):
   ...
   - Discord Developer Portal → OAuth2 → Redirects: add your `CALLBACK_URL` exactly (fixes “Invalid OAuth2 redirect_uri”)
...
- Economy bot also supports optional text-prefix commands (example: `robo balance`). If you use this, enable **Message Content Intent** for the Economy bot too.
- Economy emoji auto-seeding (currency/coinflip/dice/slots button icons) requires **Manage Guild Expressions** (and free emoji slots) for the Economy bot.
...
- Backup bot: `/backup (create|list|info|restore|delete|download|archive)`, `/template (save|list|apply)`, `/voice (set|off|status)`, `/help`.
