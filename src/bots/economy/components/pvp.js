...
function buildFightEmbed(game) {
  ...
  return new EmbedBuilder()
    ...
    .addFields({ name: 'Bet', value: game.bet > 0 ? `${formatCredits(game.bet, currency)} each` : 'No bet', inline: true })
    .setTimestamp();
}
  ...
  if (game.stage === 'challenge') {
    ...
    if (action === 'accept') {
      ...
      const [challenger, opponent] = await Promise.all([
        User.findOne({ guildId: getEconomyAccountGuildId(game.guildId), discordId: game.challengerId }),
        User.findOne({ guildId: getEconomyAccountGuildId(game.guildId), discordId: game.opponentId })
      ]);
