...
function buildChallengeEmbed({ challengerId, opponentId, bet }) {
  const embed = new EmbedBuilder()
    .setTitle('PVP Challenge')
    .setColor(0xe67e22)
    .setDescription(`<@${opponentId}>, do you accept <@${challengerId}>'s challenge?`)
    .addFields({ name: 'Bet', value: bet > 0 ? `${bet} coins each` : 'No bet', inline: true })
...
module.exports = {
  ...
  async execute(client, interaction) {
    ...
    return await interaction.reply({ embeds: [buildChallengeEmbed({ challengerId: interaction.user.id, opponentId: opponent.id, bet })], components: [row] });
  },
