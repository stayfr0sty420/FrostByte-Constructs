...
async function handleBlackjackComponent(client, interaction) {
  ...
  if (game.stage === 'insurance') {
    ...
    if (isBlackjack(game.dealer)) {
      await finishGame({ guildId: game.guildId, discordId: game.userId, game, outcome: 'dealer_blackjack' });
  ...
  if (action === 'hit') {
    ...
    if (val > 21) {
      await finishGame({ guildId: game.guildId, discordId: game.userId, game, outcome: 'player_bust' });
      client.state.setWithExpiry(client.state.blackjack, game.id, game, 60 * 1000);
      const embed = buildEmbed(game);
      await interaction.update({ embeds: [embed], components: [] }).catch(() => null);
      return true;
  ...
  const standAndResolve = async () => {
    ...
    if (d > 21) await finishGame({ guildId: game.guildId, discordId: game.userId, game, outcome: 'dealer_bust' });
    else if (p === d) await finishGame({ guildId: game.guildId, discordId: game.userId, game, outcome: 'push' });
    else if (p > d) await finishGame({ guildId: game.guildId, discordId: game.userId, game, outcome: 'player_win' });
    else await finishGame({ guildId: game.guildId, discordId: game.userId, game, outcome: 'player_loss' });
  };
